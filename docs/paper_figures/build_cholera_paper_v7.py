# -*- coding: utf-8 -*-
"""Build honest real-data cholera manuscript V7 publication figures + CRS-to-national framing.
...
P("The platform development path is important for interpreting evidence tiers. Work began with a Cross River State sentinel pilot that ingested an anonymized 2021 outbreak line-list and validated dashboard ingestion, spatial joins, and risk rendering at LGA resolution. The architecture was then generalized to a nationwide 774-LGA environmental health intelligence hub. National epidemiological figures used in this paper are compiled from publicly issued NCDC situation reports and annual summaries (state-level official reporting). Sub-national LGA resolution remains a design objective and is demonstrated empirically in the Cross River pilot rather than by fabricating a national LGA panel. The study reports two clearly separated tiers of evidence. At the national tier, the hub compiled officially "
...
"Figures 4 and 5 show the operational national interface; Figure 6 shows the pilot case and death distribution derived "
...
"disaggregated by this study into a sub-national panel. Development began with the Cross River sentinel line-list; "
"the architecture was then generalized nationwide, and national figures were compiled from publicly issued NCDC "
"situation reports and annual summaries rather than by fabricating a 774-LGA national case panel.")
...
"the dedicated Agent Explorer page used for copilot-assisted interactive dashboards. The Cross River 2021 pilot aggregate "
"is used here as a grounded demonstration dataset; the national architecture remains 774 LGAs.")
figph("Figure 11", img=os.path.join(FIG_DIR, "app_surveillance_copilot.png"), width=6.7)
caption("Figure 11: AI Surveillance Copilot (sidebar assistant) open on the national dashboard with a live surveillance query "
"spanning the 774-LGA risk context and Cross River pilot signals. The assistant is a grounded analytics aid, not an "
"autonomous outbreak-confirmation instrument.")
figph("Figure 12", img=os.path.join(FIG_DIR, "app_agent_explorer.png"), width=6.7)
caption("Figure 12: Agent Explorer dedicated page — copilot-assisted interactive dashboard workspace demonstrated with the "
"Cross River 2021 sentinel pilot line-list aggregate.")

H("6.2 Schema-Detection Benchmark",2)
P("A single successful demonstration such as Figure 12 establishes that the capability exists; it does not establish that "
...
"successfully. Results are reported in Table 7 and Figure 13.")
...
figph("Figure 13", img=os.path.join(FIG_DIR, "fig_copilot_bench.png"), width=6.8)
caption("Figure 13: Surveillance Copilot schema-detection benchmark (72 live Vertex AI invocations). (a) Effect of requiring "
...
figph("Figure 14", img=os.path.join(FIG_DIR, "diag_roadmap.png"))
caption("Figure 14: Advanced integration and automation roadmap — NCDC/SORMAS API, NASRDA satellite data hub, DHS microdata "
...
doc.save('/root/flooding-cholera-sync/docs/CHOLERA_PAPER_V7_Revised.docx')
